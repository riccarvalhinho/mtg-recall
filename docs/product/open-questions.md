...
| Q9 | A vista de um deck é uma lista de texto ou uma galeria de imagens das cartas? E com que layout — grelha, leque sobreposto, outro? | `app/deck/[id].tsx` + Fase 5 | **Em aberto:** decidido que é galeria, falta o layout. O autor vai mandar uma referência de outra app. Sem ela, qualquer desenho é adivinhação — a sobreposição e o que acontece ao tocar numa carta mudam o écran todo |
| Q10 | O OCR local aguenta uma fotografia real de cartas espalhadas — várias colunas, cartas inclinadas, só a barra do título visível? | ADR 0009 + `domain/` | **Em aberto:** a abordagem está decidida (ML Kit, ADR 0009) mas nunca foi testada contra uma foto verdadeira. O autor vai mandar uma de exemplo. O que isso decide não é se se quer, é se funciona à primeira ou se precisa de pré-processamento da imagem |
